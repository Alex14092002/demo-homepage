...
$('.slideshowsss').imagesLoaded({ background: true })
.done( function() {
  // hide loader
  $('.loader').addClass('is-loaded');
  
  // init variables
  var slideshow = $(".slideshowsss"),
    navigation = $(".navigationsss"),
    navigationItem = $(".navigation-itemsss"),
    detailItem = $(".detail-itemsss"),
    rotation,
    type = '_short';

  // prepare letters
  $('.headlinesss').each(function() {
    $(this).html($(this).text().replace(/([^\x00-\x80]|\w)/g, "<span class='lettersss'>$&</span>"));
  });
  ...
  navigationItem.each(function(index, elem) {
    ...
    TweenMax.set($(elem).find('.rotate-holdersss'), {
      text: String(index * 360 / navigationItem.length)
    });
    TweenMax.set($(elem).find('.background-holdersss'), {
      rotation: -90 - (index * 360 / navigationItem.length),
    });
  });
  ...
  function setTweenValues() {
    rotation = Number($(this).find('.rotate-holdersss').text());
  }
  ...
  function doTween(target) {
    ...
    timeline
      ...
      .staggerTo(navigationItem.find('.background-holdersss'), 0.6, {
        cycle: {
          //function that returns a value
          rotation: function(index, element) {
            return -90 - Number($(element).prev('.rotate-holdersss').text()) + rotation + type;
          }
        },
        transformOrigin: "50% 50%",
        ease: Sine.easeInOut,
      }, 0, '-=0.6')
      .staggerFromTo($('.activesss').find('.lettersss'), 0.3, {
        autoAlpha: 0,
        x: -100,
      },
      {
      ...
      .fromTo($('.activesss').find('.backgroundsss'), 0.9, {
        autoAlpha: 0,
        x: -100,
      },
      {
  ...
  TweenMax.to($('.activesss').find('.lettersss'), 0.7, { autoAlpha: 1, x: 0 });
  TweenMax.to($('.activesss').find('.backgroundsss'), 0.7, { autoAlpha: 1, x: 0 });
