...
$('.slideshows').imagesLoaded({ background: true })
.done( function() {
  // hide loader
  ...
  navigationItem.each(function(index, elem) {
    ...
    TweenMax.set($(elem).find('.rotate-holders'), {
      text: String(index * 360 / navigationItem.length)
    });
    TweenMax.set($(elem).find('.background-holders'), {
      rotation: -90 - (index * 360 / navigationItem.length),
    });
  });
  ...
  function setTweenValues() {
    rotation = Number($(this).find('.rotate-holders').text());
  }
  ...
  function doTween(target) {
    ...
    timeline
      ...
      .staggerTo(navigationItem.find('.background-holders'), 0.6, {
        cycle: {
          //function that returns a value
          rotation: function(index, element) {
            return -90 - Number($(element).prev('.rotate-holders').text()) + rotation + type;
          }
        },
        transformOrigin: "50% 50%",
        ease: Sine.easeInOut,
      }, 0, '-=0.6')
      .staggerFromTo($('.actives').find('.letters'), 0.3, {
        autoAlpha: 0,
        x: -100,
      },
      {
      ...
      .fromTo($('.actives').find('.backgrounds'), 0.9, {
        autoAlpha: 0,
        x: -100,
      },
      {
  ...
  TweenMax.to($('.actives').find('.letters'), 0.7, { autoAlpha: 1, x: 0 });
  TweenMax.to($('.actives').find('.backgrounds'), 0.7, { autoAlpha: 1, x: 0 });
